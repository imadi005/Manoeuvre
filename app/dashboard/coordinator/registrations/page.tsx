...
export default async function RegistrationsOverviewPage() {
  ...
  // Faction x Event matrix: filled count + team count per cell.
  const cell = (factionId: string, eventSlug: string) => {
    const event = events.find((e) => e.slug === eventSlug)!;
    if (event.teamConfig || event.subEvents) {
      const eventTeams = teams.filter((t) => t.factionId === factionId && t.eventSlug === eventSlug);
        ...
        <div className="relative mx-auto max-w-6xl">
          ...
          <div className="mt-12">
            ...
            <div className="flex flex-col gap-3">
              {factions.map((f) => {
                const factionTeams = teams.filter((t) => t.factionId === f.id);
